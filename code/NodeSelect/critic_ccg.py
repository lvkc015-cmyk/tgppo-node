...
class CriticCCG(nn.Module):
    """Critic network with a candidate-conditioned graph branch."""
    ...
    def forward(self, cands_state_mat, node_state, mip_state, norm_cons, norm_edge_idx, norm_edge_attr,
        ...
        # Codex note: critic reads the same candidate-conditioned graph context so value estimation matches the actor input.
        ...
        masked_sum = (transformed_features * valid_mask).sum(dim=1)
