...
class Memory:
    ...
    def __init__(self, batch_size, state_dims, device, logger=None):
        ...
        # 【新增】：接收候选结点的特征维度
        self.node_cand_dim = state_dims.get("node_cand_dim", self.var_dim)
    ...
    def _validate_inputs(self, cands_state, mip_state, node_state, action, reward, done, log_prob, value):
        ...
        if cands_state.size(1) != self.node_cand_dim:
            raise ValueError(f"cands_state must have {self.node_cand_dim} columns, got {cands_state.size(1)}")
        ...
        # 1. 提取 action 的原生 int 值 (兼容 Tensor 和 Numpy)
        if hasattr(action, 'item'): 
            action_val = int(action.item())
        else: 
            action_val = int(action)

        if action_val < 0 or action_val >= cands_state.size(0):
            raise ValueError(f"Invalid action index: {action_val}, total candidates: {cands_state.size(0)}")

        # 2. 提取 reward, log_prob, value 的原生 float 值
        if hasattr(reward, 'item'): reward = float(reward.item())
        else: reward = float(reward)
            
        if hasattr(log_prob, 'item'): log_prob = float(log_prob.item())
        else: log_prob = float(log_prob)
        ...
        if np.isnan(reward) or np.isinf(reward):
            raise ValueError(f"reward must be finite number, got {reward}")
        if np.isnan(log_prob) or np.isinf(log_prob):
            raise ValueError(f"log_prob must be finite number, got {log_prob}")
        if np.isnan(value) or np.isinf(value):
            raise ValueError(f"value must be finite number, got {value}")
        ...
        for start_idx in range(0, n_states, batch_size):
            ...
            for cs in batch_cands_states:
                ...
                if padding_len > 0:
                    ...
                    padded_cs = T.cat([
                        cs_on_device, 
                        T.zeros(padding_len, self.node_cand_dim, device=self.device)
                    ], dim=0)
                    mask = T.cat([
    ...
    def export_dict(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            "node_cand_dim": self.node_cand_dim, # 【新增】这一行
    ...
    def import_dict(self, payload: Dict[str, Any]):
        ...
        for i in range(n):
            ...
            # Minimal validation
            if cs.dim() != 2 or cs.size(1) != self.node_cand_dim:
